...
export const postJoin = async (req, res, next) => {
    ...
    if (password != password2) {
        ...
        try {
            const user = await User({name, email}); //create: create user + save in db
...
export const githubLoginCallback = async (_, __, profile, cb) => { //this _ name means that this variable is not used
    ...
    try {
        ...
        if (user) {
            ...
            const newUser = await User.create({email, name, githubId: id, avatarUrl});
...
export const getEditProfile = (req, res) => res.render(
    "editProfile",
...
export const postChangePassword = async (req, res) => {
    ...
    try {
            if (newPassword !== newPassword1) {
                res.status(400); //200: google autocomplete the field
